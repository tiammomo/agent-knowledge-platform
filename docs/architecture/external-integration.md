...
> 签名固定 Tenant Principal、Query Space 前置过滤、REST/TypeScript/Python SDK 和 MCP Adapter；
> 外部 PDP、动态多 Tenant、外部 Ingestion Connector、集成控制面、Outbox relay 与 webhook 注册
> 尚未实现。本设计中的目标端点和控制面对象不能当作当前 API。
...
当前固定单 Tenant 子集要求签名 claim（默认 `akep_tenant`，可由 `OIDC_TENANT_CLAIM` 改名）的
值与部署 `AKEP_TENANT_ID` 完全一致。Core 会生成本地 Query AuthorizationPlan，把允许 Space
下推到 Published 元数据和 PostgreSQL Passage 排序/候选上限之前，并将授权摘要绑定游标。它还
不是 Integration Registry 映射、外部 PDP 或共享多租户授权服务。
...
3. **I2 动态租户上下文/PDP**：全表 Tenant/RLS、固定部署签名 Tenant Principal 与 Query Space
   前置过滤已完成；控制面 Tenant 映射、事务级动态上下文、外部 PDP 与全链路隔离完成后才允许
   共享运行时。
