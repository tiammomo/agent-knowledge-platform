...
当前参考实现按进程配置固定单一 Tenant。数据库事实表、复合约束、RLS、签名固定 Tenant
Principal 与 Query Space 前置过滤已完成阶段性落地，但控制面 Tenant 映射、动态事务 Tenant、
外部 PDP、缓存、任务和管理面尚未形成完整租户边界。仅为每个团队创建一个 Space，或在 SQL 中
添加可选 tenant 条件，不能抵御 IDOR、连接池上下文泄漏、全局检索/缓存、Outbox/Worker 混淆和
管理员越权。
