...
> [!WARNING]
> 当前 Core 从进程配置读取固定 `AKEP_TENANT_ID`，Principal 不携带可信 tenant，上述表也并非
> 全部有 `tenant_id`，且未启用 PostgreSQL RLS。因此当前只能“一部署/实例对应一个受控租户”，
> 不能通过创建多个 Space 宣称已经实现多租户隔离。
...
所有租户拥有的数据表必须有非空 `tenant_id`，所有 Space 资源还必须有 `space_id`。当前需要
补齐的重点包括 Contribution workflow、LifecycleEvent、Exposure/Usage/Feedback Receipt、
Outbox、Workflow idempotency 和任何后续 Connector/Subscription 表。
...
| 表/能力 | 当前问题 | 目标 |
| --- | --- | --- |
| `contribution.workflow` | 无 `tenant_id`，主键/幂等只按主体 | 增加 tenant，复合唯一键/FK |
| `governance.lifecycle_event` | 无 `tenant_id` | tenant + space + event 复合身份 |
| Exposure/Usage/Feedback Receipt | 多表无 tenant 列，tenant 只隐含在 JSON/进程配置 | tenant/space 独立列并参与索引/RLS |
| `platform.outbox_event` | 无 tenant/space | 增加 tenant、space、classification、delivery priority |
| PostgreSQL role | 未启用 RLS，应用通过 SQL 显式过滤 | Runtime RLS + PDP 双层 |
...
1. API 每个数据库事务先从已认证 Principal 执行
   `SELECT set_config('app.tenant_id', $1, true)`；第三个参数 `true` 使上下文只在当前事务有效。
...
6. 缺少 tenant 上下文时 `current_setting(..., true)` 为 NULL，策略默认拒绝。
...
- 为缺失表新增 `tenant_id`/`space_id`，回填当前配置租户。
- 重建 PK/Unique/FK/索引，消除跨租户全局唯一。
- 所有 Store 方法显式接受 TenantContext，不再直接读取全局 config。

### M2：可信 Principal 与事务上下文

- IdP/client → tenant mapping；Principal 增加 tenant/actor/integration。
- 每事务 `set_config(..., true)`，runtime role 非 owner；全表 ENABLE + FORCE RLS。
- 后台 job、迁移、Console 和观测路径完成同样改造。

### M3：PDP 与检索/缓存

- 持久化单调 Policy Epoch；PDP 输出 AuthorizationPlan。
- Query pre-filter、外部搜索 adapter、缓存键、Receipt 和 Blob 全面租户化。
- 完成负向/侧信道/连接池/备份恢复测试。
...
M1–M3 完成前，系统仍按单租户部署；`tenant_id` 字段存在不等于多租户可用。
