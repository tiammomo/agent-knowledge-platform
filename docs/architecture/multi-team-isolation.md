...
> [!WARNING]
> 当前 Core 仍从进程配置读取固定 `AKEP_TENANT_ID`，Principal 不携带可信 tenant。17 张租户
> 事实表已经统一 `tenant_id` 并启用 `ENABLE/FORCE RLS`，但这是“一部署/进程对应一个受控
> Tenant”的数据库纵深防御。不能仅凭 RLS 或创建多个 Space 宣称共享多租户已经实现。
...
| PostgreSQL role | production readiness 强制非 owner、非 superuser、非 `BYPASSRLS` | 动态 Principal Tenant、事务级上下文与外部 PDP 双层 |
