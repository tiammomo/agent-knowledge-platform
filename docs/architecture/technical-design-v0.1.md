...
- [外部系统接入](external-integration.md)
...
| 模块 | 职责 | 关键边界 |
| --- | --- | --- |
| Integration Control Plane | Integration/Connector、Owner、身份映射、配额、轮换和停用 | 不存明文 secret，不授予治理权限 |
...
| Maintenance | Owner Directory、来源 checkpoint、due/conflict/incident queue 和 SLA | 只创建任务、证据或 Contribution |
...
多团队目标态中，Tenant 是身份/法律/密钥硬边界，Space 是 Tenant 内治理边界。可信 tenant
只能从验证后的身份与控制面映射生成；全表 RLS、检索前过滤、对象/缓存/队列隔离和 Shared Space
规则见[多团队隔离设计](multi-team-isolation.md)。
