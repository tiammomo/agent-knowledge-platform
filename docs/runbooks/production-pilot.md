...
导出与 MCP 接入；固定部署的签名 Tenant Principal 和 Query Space 前置过滤也已落地。控制面
动态 Tenant 映射、事务级 Tenant、外部 PDP/完整策略下推、对象存储和外部恶意文件扫描仍是
扩大部署范围前的门禁。
...
读取。`OIDC_TENANT_CLAIM` 指向一个已签名 JWT claim；其值必须是绝对 URI，并与
`AKEP_TENANT_ID` 完全一致，缺失、格式错误或不匹配都会按无效身份拒绝。不要从代理 header、
URL 或请求 body 注入 Tenant。反向代理必须终止 HTTPS、限制请求体，并只在可信代理链路上设置
`TRUST_PROXY=true`。
...
同一 token 还必须包含与部署一致的签名 Tenant：

```json
{"akep_tenant":"https://knowledge.example.com/tenants/acme"}
```

Query/ContextPack 会把这个 Tenant、主体、Space、purpose、obligation 与 `policyEpoch` 编译成本地
AuthorizationPlan。精确 Space 集合在 Published 元数据读取和 Passage SQL 排序/候选上限之前
过滤，continuation cursor 也绑定授权摘要；换主体、Space 或策略水位后不能继续使用旧游标。
...
1. 在现有固定签名 Tenant Principal、全表 RLS 与 Query Space 前置过滤上，完成控制面 Tenant
   映射、事务级动态上下文、外部 PDP/完整策略谓词，以及对象/缓存/队列/统计侧信道验收；单个
   配置 Tenant 不是通用多租户隔离。
