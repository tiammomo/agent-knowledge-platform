...
已经接入；`NODE_ENV=production` 会拒绝 development auth。当前 17 张租户事实表已经启用
`ENABLE/FORCE RLS`，Core 连接池绑定部署 `AKEP_TENANT_ID`，production readiness 强制使用
owner 管理的单 Tenant runtime role，且该角色必须非 owner、非 superuser、非 `BYPASSRLS`。
可信动态 Tenant Principal、外部 PDP
和生产管理面会话仍未完成，不得将 `dev-*` 令牌或当前 Web 开发 bundle 暴露到生产网络。
