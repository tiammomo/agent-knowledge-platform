...
| 查询 | Published Channel lexical/exact Query、授权绑定游标、预算化 ContextPack；Space 在元数据读取与 SQL 排序/上限前过滤 |
...
| Web Console | 按真实工作流状态生成职责化下一步；知识检索展示授权/策略/投影证据、来源与适用边界；Agent 接入提供无凭证公开预检；另有贡献、审核、发布治理、效果证据、设置和五步真实引导 |
...
打开 `http://localhost:33005`。首次访问会启动五步引导，并通过真实 API 完成：
...
| 地址 | 用途 |
| --- | --- |
| `http://localhost:33005` | Web Console 与统一 Web Origin |
| `http://localhost:38085` | Core 直连调试 |
| `http://localhost:38085/.well-known/akep` | Capability Discovery |
| `http://localhost:38085/health/ready` | 就绪检查 |

### 推荐使用路径

1. 首次打开 Console，先完成五步真实引导，确认本地环境能走通
   Discovery → Candidate → Review → Publish → Cited Query。
2. 回到总览的“现在最值得处理”，按 Contributor、Curator、Publisher、Reader / Agent 集成人员等
   职责处理真实工作流状态。当前本地页面使用多个开发身份演示职责分离，并非生产登录用户的个性化任务队列。
3. 选择一个高价值场景和单个测试 Space：Contributor 提交候选，Curator 核验证据，Publisher 决定
   是否发布；不要用同一身份或自动脚本绕过职责分离。
4. 在知识库执行带引用查询，核对授权 Space、purpose、Policy Epoch、投影/索引水位、来源、适用
   范围和义务。排序分只表示本次查询相关性；零结果也不等于平台确认答案不存在。
5. 在 Agent 接入页运行无凭证公开预检，再从 Agent 的实际部署网络使用 audience-bound 短期 token
   重做 Discovery、认证负向测试、Space/purpose 授权、Query/ContextPack 和 Usage/Feedback 闭环。

公开预检只检查当前浏览器到公开健康端点、Capability、OAuth 资源元数据与关键 Schema 的连通性和
基本一致性；检查不会发送 `Authorization`，也不验证真实 Agent 的 token、网络、Space 或 purpose。
...
另开终端运行 `pnpm dev:web`，访问 `http://localhost:33005`。完整步骤、开发身份和排障见
...
# 为 QuantPilot 创建可长期复核的 50 题受治理知识（数据会保留）
pnpm seed:quantpilot-acceptance-50 -- --output=/absolute/path/to/manifest.json
```

`pnpm test:integration` 默认创建并删除独立的 `akep_test_<uuid>` 数据库，不污染本地开发库；
`TEST_DATABASE_URL` 表示调用方接管隔离与清理，`AKEP_KEEP_TEST_DATABASE=true` 只用于短期故障取证。
`smoke:web` 会写入它所连接的数据库，应只对一次性 smoke 环境运行。
`seed:quantpilot-acceptance-50` 只写入专用 Space
`https://knowledge.local/spaces/quantpilot-acceptance-50-v1`，并以确定性 record/title 幂等重跑；它用于
QuantPilot、ModelPort、Memory 与 AKEP 的持久闭环验收，不是无状态 smoke，也不会自动删除知识和回执。
...
- [QuantPilot 接入](docs/architecture/quantpilot-integration.md)：AKEP ContextPack、Citation/Usage、ModelPort/Qwen 分工与解耦边界。
- [Memory/Knowledge 组合边界](docs/architecture/memory-knowledge-composition.md)：三方对象所有权、联合 receipt 与结果反馈语义。

本地 `dev-*` token 只用于非生产角色演示。OIDC 模式下必须使用短期、audience-bound token；
签名 `akep_tenant`（可通过 `OIDC_TENANT_CLAIM` 改名）必须是与部署 `AKEP_TENANT_ID`
完全一致的绝对 URI，签名 `akep_obligations` 只发放调用方真正能够履行的义务。
...
当前运行时仍是单租户进程模型：OIDC Principal 必须携带与部署一致的签名 Tenant，Core 将专用
连接池固定绑定到同一 `AKEP_TENANT_ID`。全部 17 张租户事实表具有非空 `tenant_id`、租户复合
约束与 PostgreSQL `ENABLE/FORCE RLS`；production readiness 会校验 owner 管理的数据库角色 →
Tenant 绑定，并拒绝 table owner、superuser 和 `BYPASSRLS` 运行角色。Query 会把本地编译的
Tenant/subject/Space/purpose/obligation/policy-epoch `AuthorizationPlan` 绑定到游标与曝光决策，
显式/精确 Space 集合在 Published 元数据读取和 SQL 排序/候选上限前下推。

这些能力解决的是单租户部署内的身份、查询和数据库纵深防御，不等于共享多租户运行时。
控制面 issuer/client → Tenant 映射、每事务动态 Tenant、外部 PDP、完整策略谓词下推、Space
共享工作流、对象/缓存/队列隔离和统计侧信道验收仍未完成。
...
| QuantPilot 专项接入 | [QuantPilot 接入设计](docs/architecture/quantpilot-integration.md) |
| Memory、Knowledge 与任务编排器组合 | [组合边界](docs/architecture/memory-knowledge-composition.md) |
...
| 数据维护与测试隔离 | [数据维护运行手册](docs/runbooks/data-maintenance.md) |

## 生产边界

当前实现可以作为受控单租户隔离试点的技术基线，但扩大部署前至少还要完成：

1. 在现有签名 Tenant Principal、全表 Tenant/RLS 与 Query Space 前置过滤基线上，完成控制面
   Principal → Tenant 映射、事务级动态 Tenant、外部 PDP/完整策略谓词下推，以及对象/缓存/
   队列/统计侧信道的完整越权验收。
...
- 动态多租户 TenantContext、外部 PDP、对象/缓存/队列隔离与 Shared Space 的落地顺序。
